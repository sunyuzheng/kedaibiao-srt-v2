...
相比 v7 的改动：
  ...
  5. call_llm 加超时保护
  6. 支持多 LLM 提供商：Anthropic Claude / OpenAI / Google Gemini

用法：
  from tools.correct.correct_srt import correct_file
  correct_file(qwen_path, episode_seeds=["刘嘉", "Superlinear Academy"])
  correct_file(qwen_path, model="gpt-4o")
  correct_file(qwen_path, model="gemini-1.5-pro")
"""
...
# 从 .env 文件加载所有 KEY=VALUE 到 os.environ（不覆盖已有值）
...
# 将 API key 汇总到 _API_KEYS dict（供各 provider 函数使用）
for _k in _KEY_NAMES:
    if os.environ.get(_k):
        _API_KEYS[_k] = os.environ[_k]

# ── 配置 ─────────────────────────────────────────────────────────────────────
_ROOT = Path(__file__).parent.parent.parent
_VOCAB_FILE = _ROOT / "data" / "channel_vocab.json"

DEFAULT_MODEL      = os.environ.get("DEFAULT_CORRECTION_MODEL", "claude-haiku-4-5-20251001")
...
def _detect_provider(model: str) -> str:
    ...
    raise ValueError(
        f"无法识别模型提供商: {model}\n"
        "请使用 claude-* / gpt-* / gemini-* 开头的模型名"
    )


def _call_anthropic(system: str, user: str, model: str) -> str:
    api_key = _API_KEYS.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        raise RuntimeError("未找到 ANTHROPIC_API_KEY，请在 .env 文件中配置")
    if "anthropic" not in _llm_clients:
        try:
            import anthropic as _anthropic
        except ImportError:
            raise RuntimeError("未安装 anthropic，请运行: pip install anthropic")
        _llm_clients["anthropic"] = _anthropic.Anthropic(api_key=api_key, timeout=API_TIMEOUT)
    client = _llm_clients["anthropic"]
...
def _call_openai(system: str, user: str, model: str) -> str:
    api_key = _API_KEYS.get("OPENAI_API_KEY") or os.environ.get("OPENAI_API_KEY")
    if not api_key:
        raise RuntimeError("未找到 OPENAI_API_KEY，请在 .env 文件中配置")
    if "openai" not in _llm_clients:
        try:
            from openai import OpenAI
        except ImportError:
            raise RuntimeError("未安装 openai，请运行: pip install openai")
        _llm_clients["openai"] = OpenAI(api_key=api_key, timeout=API_TIMEOUT)
    client = _llm_clients["openai"]
...
def _call_gemini(system: str, user: str, model: str) -> str:
    api_key = (
        _API_KEYS.get("GOOGLE_API_KEY")
        or _API_KEYS.get("GEMINI_API_KEY")
        or os.environ.get("GOOGLE_API_KEY")
        or os.environ.get("GEMINI_API_KEY")
    )
    if not api_key:
        raise RuntimeError("未找到 GOOGLE_API_KEY 或 GEMINI_API_KEY，请在 .env 文件中配置")
    try:
...
def call_llm(system: str, user: str, model: str) -> str:
    """统一 LLM 调用入口，自动识别提供商，内置重试"""
    provider = _detect_provider(model)
    _RETRY_WAITS = [10, 30, 60]
    _RETRYABLE = ("overload", "rate", "timeout", "connection", "429", "529", "503")
    ...
    for wait in [0] + _RETRY_WAITS:
        ...
        try:
            ...
            err_str = str(e).lower()
            print(f"  [API 错误: {type(e).__name__}: {str(e)[:60]}]", flush=True)
            last_exc = e
            if any(k in err_str for k in _RETRYABLE):
                continue  # 可重试
            raise        # 不可重试（如认证错误），立即抛出
...
def full_doc_llm_scan(
    chunks: list[dict], model: str, verbose: bool = False
) -> tuple[list[dict], int]:
...
def correct_file(
    ...
    corrected, scan_corrections = full_doc_llm_scan(corrected, model, verbose=verbose)
    if scan_corrections:
